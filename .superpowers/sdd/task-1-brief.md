...
- Modify: `Cargo.toml` (workspace members + `url` and `base64` deps)
...
Add `proxy-sub` to workspace members and `url = "2"`, `base64 = "0.22"` to workspace dependencies:
...
Add at the end of the file, after the `WarpInstance` impl block:
