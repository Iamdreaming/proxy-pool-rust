...
2. **Response structs** — `StatusResponse`, `PoolStatus`, `ProxiesResponse`, `SimpleResponse`, `RefreshResponse`; xray lifecycle status serializes shared `proxy_core::xray_status::XrayStatusSnapshot`; subscription source ops serializes `proxy_sub::ops` snapshots and reports
